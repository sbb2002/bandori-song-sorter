...
| `assets/albums/<band>/*.webp` | 앨범 커버 | `data/*.yaml`의 `img_url` (※ 현 v2 화면 미사용, **legacy 보존**) |
...
- **동시 수정**: `static/js/script.js` 2곳(아이콘·단체사진 경로) + 전 `data/*.yaml` `img_url` + `build.py` 재빌드(곡 488). 무결성 검증 통과(고유 `img_url` 40개 전부 추적 파일로 연결).
