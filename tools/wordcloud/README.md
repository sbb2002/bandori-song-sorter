...
뽑아 워드클라우드용 `wordcloud/<band>.yaml`(커밋·사용자 편집)을 생성한다.
...
## 사용
```bash
python -m pip install -r tools/wordcloud/requirements.txt
python tools/wordcloud/build_keywords.py            # 전 밴드 → wordcloud/*.yaml
python tools/wordcloud/build_keywords.py --debug    # yaml 미기록, 상위 명사 확인
python tools/wordcloud/build_keywords.py --band mygo --no-translate
```
