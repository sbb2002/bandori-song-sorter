"""cluster/songs_top10.csv → 지각 축 음원맵 (cluster/audio_map.json). v3 채택본.

축 재정의 결과(docs/report/cluster-correlation) 채택:
...
사용: python tools/cluster/build_perceptual_map.py [--cache audio_cache|audio_full]
"""
...
OUT = Path("cluster/audio_map.json")
SR = 22050
...
#   퍼져 상수 통용 불가, HPSS 선율밝기도 valence 무상관 — docs/report/cluster-correlation).
