...
// [EMOI-MAP 전용] 딥스페이스(near-black) 배경에서 톤이 묻히는 밴드 색만 지도 한정으로 가시성 보정.
// 전역 BAND_COLORS(워드클라우드·리스트 등 공유)는 불변 — 여기 오버라이드는 16-audiomap.js 안에서만 사용.
// ave_mujica(#881144)=어두운 와인색 → 배경에 강조 안 됨 → 밝은 로즈 마젠타(정체성 유지, L·S↑).
const CL_COLOR_OVERRIDE = { ave_mujica: '#e64c8c' };
...
function _clDynLevel(now) {              // 현재 시각 intensity → subdivision 레벨(0박·1·2)
    ...
    return Math.min(CL_DYN_MAX, lvl);
}
...
function _clDraw() {
    ...
    const songMark = (s, val, base) => {   // 재생 곡=가장 밝게 / 재생 중이면 나머지=약간 어둡게 / 호버 흐림=글로우 억제
        ...
        const star = faint ? col : _clPulseColor(col);             // 어두운 밴드색도 딥스페이스 위 가시
        return {
