...
| [**feature-validity-extraction.md**](feature-validity-extraction.md) | 오디오 피처 **유효성**(3중 렌즈) | "한 변수씩 보면 놓친다" — Spotify 114,000곡 + 우리 285곡에서 단·이·다변량 3중 렌즈로 장르 구분력 검증. 단변량 순위가 렌즈에 따라 뒤집힘(loudness↔energy 중복 저평가·popularity 반전·Simpson's paradox) → harmonic_ratio·energy_proxy(rms+contrast+flux) 우선순위(축 개편은 전곡 재검증 후) |
...
- `featval_fig4_perm_importance_local.png` — 우리 샘플 285곡 다변량 permutation importance(스펙트럼 형태 지표군 중복)
- `featval_fig5_simpsons_paradox_loudness_danceability.png` — loudness↔danceability 장르별 부호반전(39/114, Simpson's paradox)
- `featval_fig6_rank_shift_slope.png` — 단변량 η² 순위 → 다변량 PI 순위 이동(렌즈에 따른 순위 반전) ★
- `featval_fig7_vif_vs_importance.png` — VIF vs permutation importance, Spotify·우리 샘플 두 데이터셋 공통(중복 → 저평가) ★
