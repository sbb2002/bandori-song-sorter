...
### 언제 research로 쓰나
- **report에서 "꽤 고민한 탐색"만 승격한다.** 가설 → 여러 시도(실패 포함) → 전환 → 귀결의 서사가 있을 때. 단순 완료 기록·수치 덤프는 report/done에 남기고 승격하지 않는다.
- **1 논문 = 1 여정**(하나의 질문에서 출발해 하나의 결론으로). 진행 중인 조사는 `working/report/<주제>/`에 두고, **완료된 뒤** 종합·정제해 research로 올린다.
