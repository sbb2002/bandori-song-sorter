...
마지막 갱신: ux-02.md #6(밴드 셀렉터 진행률 링·SVG) 완료·커밋 후 남은 5건 재번호 (2026-06-24)

> **ux-02.md 1·2·6·7번 완료**: 1·2(히트맵 셀렉터순 / 최애 스코어링) 세션 8·9, **7(재생 중 곡 강조) 세션 10**, **6(밴드 셀렉터 진행률 링) 세션 11**(SVG stroke 채택, conic은 `feature/ux-02-ring-conic` 백업). 상세는 [done.md](done.md). 옵션 A(랭크순)는 `feature/ux-02-opt-a` 백업.
> 아래는 남은 5건을 **구현 난이도 낮고 기존 기능을 덜 해치는 순**으로 유지.
...
| 순 | 작업 | 난이도 | 기존기능 리스크 |
|----|------|--------|-----------------|
| 1 | 티어 팝업 Comment 란 | 중 | 낮~중(저장 스키마 확장) |
| 2 | youtube_rss 자동화 | 중 | 낮음(앱 런타임과 분리) |
| 3 | 데이터 품질 검수 | 중(수작업) | 중(곡 데이터 변경→회귀) |
| 4 | 진행도 Save/Load | 중~높 | 높음(진행 덮어쓰기→손실) |
| 5 | 한국 지역락 대응 | 높(불확실) | 중(감지·대체 미정) |

원칙: **데이터·저장 안 건드리는 순수 UI 추가(1) → 앱과 분리된 인프라(2) → 곡 데이터 변경(3) → 저장 덮어쓰기(4) → 방안 불확실(5).**
...
### 1. 티어 팝업 Comment 란 (ux-02.md #3) — 난이도 중 · 리스크 낮~중
티어 팝업에 코멘트 입력란 → 티어 확정 시 곡 리스트 티어뱃지 옆에 **말풍선 뱃지** 생성. 티어 미확정이면 코멘트는 유지하되 뱃지 미생성. 곡에 호버 또는 뱃지 클릭 시 코멘트 표시.
- 구현 포인트: localStorage에 comment 저장(`ranks`와 별도 키 또는 스키마 확장). 곡 리스트 렌더에 뱃지·툴팁. 추가 기능이라 기존 무영향, 단 저장 포맷 버전 관리 주의(#4 Save/Load와 직렬화 범위 함께 고려).

### 2. 자동화 — youtube_rss GitHub Actions 크론 (미구현) — 난이도 중 · 리스크 낮음
...
- **추천(미구현)**:
  ...
  - ※ 데이터품질 툴(#3)과 oEmbed·길이 스크랩 로직 공유 가능 → 공통 유틸로 묶기.

### 3. 데이터 품질 검수 (사용자 직접 진행 예정) — 난이도 중(수작업) · 리스크 중(회귀)
...
### 4. 진행도 Save/Load (ux-02.md #4) — 난이도 중~높 · 리스크 높음 · 후순위
...
- **후순위**: 디씨 json 첨부가 금기/헤비하면 이 기능 반려 가능. 구현성·안정성·커뮤니티 배포가능성 선검토 필요. (#1 Comment까지 저장 범위에 포함할지 직렬화 스키마 함께 확정.)

### 5. 한국 지역락 노래 대응 (ux-02.md #5) — 난이도 높(불확실) · 리스크 중
일부 곡이 한국 지역락일 수 있음 → 대응책 필요.
- 미정 영역: 지역락 **감지 방법**(클라이언트에서 판별 난해), **대체 링크/표기** 정책. 방안 구상부터 필요. #3 데이터 품질 검수와 oEmbed 점검 로직 일부 공유 가능.
