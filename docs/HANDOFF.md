...
> 2026-06-27 (`feature/emoi-cloud`): #2 **키워드 추출 파이프라인 완료** — `tools/wordcloud/`로 10밴드 `wordcloud/<band>.yaml` 생성·커밋. 커버 제외·번안 우선 정렬(+MT 보완). 남은 건 사용자 ko 검수 + 렌더. 상세는 2번 섹션.
...
- **🔄 부분완료 — 키워드 추출 파이프라인(`feature/emoi-cloud`, 2026-06-27)**. `tools/wordcloud/`(README 참조). 렌더는 미착수.
  - **✅ 완료**
  ...
  - **⬜ 남은 것**
    - **(2-a) yaml ko 사용자 검수**: 정렬/MT 초안의 오역·노이즈(특히 HHW 가타카나 의성어, 저빈도 weight 2 오결합) 수정. `# 기계번역 초안` 주석이 우선 점검 대상.
    - **(2-b) 렌더**: `wordcloud/*.yaml` → 우패널 3번째 탭 워드클라우드(라이브러리·후렴 빈도 보정 결정). '밴드소개' 텍스트는 미수집이라 **워드클라우드 단독부터**. 가독성 나쁘면 원안(유튜브 프레임 가로분할) 폴백.
  - 재생성: 가사 수정 시 `python tools/wordcloud/build_keywords.py`(멱등). mygo `春日影` 2버전은 1개만 채워 빈 곡 스킵으로 자동 dedup.
