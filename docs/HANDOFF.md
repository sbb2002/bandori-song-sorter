...
- **현재 상태**: 1번에서 `assets/lyrics/<band>.md` 가사 템플릿(10밴드 TOP10) 생성 완료 → **사용자가 가사 채우는 중**. 채워지면 형태소→`wordcloud/<band>.yaml`(원문 .md 삭제)→렌더 착수. mygo `春日影`는 2버전이 들어있으나 가사 동일하니 1개만 채울 것.
