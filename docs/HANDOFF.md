...
### 3. ux-02 후속: Download 이미지에 밴드별 신뢰도(progress bar + 수치) — 미구현
부분평가 시 평가곡이 적은(n≈τ) 밴드는 베테랑이어도 점수가 눌리므로(위 #0 설계 결정의 수용된 귀결), **신뢰도를 간접 표시**해 사용자가 "점수 낮음 = 덜 평가함"을 인지하게 함.
- **무엇을**: 밴드별 **작은 프로그레스 바 + 수치**를 Download 이미지(`buildCaptureDOM`)에 추가.
- **권장 인코딩(구현 시 확정)**: 바 = **`w(n) = 1 - exp(-n/τ)`** (공식이 실제 쓰는 신뢰 가중치 0~1). 수치 = **평가곡수 `n_k`**(또는 `n_k/밴드곡수`).
  - ⚠️ 단순 "완성도(`n/전체곡수`)" 바는 지양: 1곡 밴드는 그 1곡을 평가하면 완성도 100%여도 `w=0.25`라 점수는 눌림 → 완성도와 점수가 모순돼 오해 유발. **신뢰 바는 `w(n)` 기준이어야 일관됨.**
- **성격**: 이 표시는 점수·1위 선정을 **바꾸지 않음**(설명 전용). 부분평가 중 1위 이미지가 "진짜 최애"가 아닐 수 있고, 바는 그 이유를 보여줄 뿐.
- **구현 메모**: `core.bandScores`가 이미 `{score, raw, n}`를 반환 → 바엔 `1-exp(-n/τ)`, 수치엔 `n` 사용. core에 `confidence(n)` 헬퍼 추가 고려(테스트 가능). τ는 `C.SCORE_TAU` 재사용.
- UI 디테일(위치·크기·색·표시 포맷)은 구현 시 결정 — 사용자 방침: "UI는 구현할 때 고민."
