...
# 세션 36 — EMOI-MAP 밴드 네트워크 메시(quantum.html 디자인 레퍼런스 차용) (2026-07-09, `feature/emoi-pulse-signature`)

사용자가 `side-project/emoi-map-design-candidate/quantum.html`(Three.js 3D 뉴럴 네트워크 데모)을 디자인
레퍼런스로 검토 요청 → 풀 3D 전환은 UX 복잡도·모바일 부담(상시 블룸 포스트프로세싱·오버드로우) 대비 실익이
낮아 **보류 결정**. 대신 "노드-엣지 네트워크 그래프를 점선으로" 컨셉만 차용해 기존 2D ECharts EMOI-MAP에
경량으로 이식.

## 결정
- **ALL 개요에선 표시 안 함**(밴드가 중심으로 뭉쳐 있어 지저분해짐), **밴드 포커스 시에만** 표시.
- 연결 규칙 미확정 상태로 시작 → **k-최근접(곡당 3개, `CL_MESH_K`) 기본값**으로 구현, 조정은 상수 하나만
  바꾸면 됨.
- 모양은 quantum.html처럼 "네트워크 메시"(점 하나가 여러 이웃과 동시에 연결) — 최소신장트리 같은 성긴
  트리가 아니라 촘촘한 그물.

## 구현
- `static/js/functions/16-audiomap.js`: `_clMeshEdges(list, c, k)` 신규(O(n²) 전수 거리 계산 — 밴드당
  최대 ~90곡이라 성능 이슈 없음, 대칭 쌍은 `Set` 키로 중복 제거). `_clDraw()`의 `focus` 분기에서
  센트로이드 기준 상대좌표로 계산 후, 신규 `id:'mesh'` 커스텀 시리즈(`type:'custom'`, `z:1`, `songs`
  시리즈 z:2보다 뒤에 깔림)의 `renderItem`이 `api.coord()`로 데이터→픽셀 변환해 점선(`lineDash:[3,4]`)을
  그림 — 기존 `cent-arrows` 커스텀 시리즈와 동일 패턴 재사용. 색은 밴드 퍼스널 컬러, opacity 0.22로 별
  점을 가리지 않게.
- 검증: morfonica 포커스 시 109개 간선, ALL 모드는 0개(빈 배열) 확인. 이건 애니메이션 없는 정적 커스텀
  시리즈라 headless 스크린샷도 정상 캡처됨(세션 35 펄스 애니메이션과 달리 캡처 문제 없음).
- **밝기 조정**(사용자 피드백 "너무 희미함"): `opacity 0.22→0.55` · `lineWidth 1→1.4` · 색을 `_clBandColor(focus)`
  원색 대신 `_clPulseColor(_clBandColor(focus))`(펄스 등에 쓰는 밝기 보정 함수)로 교체 — ave_mujica처럼 어두운
  원색(#881144)도 배경 위에서 선명하게 보이도록. morfonica·ave_mujica 두 밴드로 재검증(headless 스크린샷).

## 펄스 채널 이상 발견 → 다음 단계는 다른 로컬
사용자가 EMOI-MAP을 쓰다가 **ave_mujica(헤비메탈 성향 밴드)의 펄스가 acoustic 모양(6엽 물결링)으로 나오는
게 이상하다**고 지적. 실제 데이터로 확인해보니 진짜 이상 패턴이었다: ave_mujica 29곡 중 **14곡(48%)이
acoustic 채널** — 전곡 평균(29.1%)보다 훨씬 높다. 원인 추정(z-score 기준):
- `Symbol III : ▽` — acoustic z=+2.87, bright z=**−3.19**
- `碧い瞳の中に` — acoustic z=+1.37, bright z=**−2.09**
- `in your blue eyes` — acoustic z=+1.39, bright z=**−1.85**

`harmonic_ratio`(HPSS)는 "밝음/거칢"이 아니라 "톤(지속음) vs 타격(과도음) 비율"을 재는 지표라, 지속형
디스토션 파워코드는 청감상 거칠어도 하모닉 우세로 잡힐 수 있다. 여기에 ave_mujica의 어두운(저음 위주)
믹스 성향이 밝기군(`centroid`/`rolloff`/`zcr`/`flatness`)을 극단적으로 깎아, 3채널(acoustic/bright/shimmer)
상대비교에서 acoustic이 기본 승리하는 구조. 세션 33 연구(`acousticness_proxy = harmonic_ratio 주도`
한계)와 같은 맥락이지만, morfonica(바이올린)에서는 가설과 맞았던 지표가 **디스토션 밴드에서는 반대로
오작동할 수 있다**는 새 관찰.

**다음 단계(다른 로컬, 오디오 필요 없음)**: EMOI-MAP이 쓰는 7항목(x축 `contrast`·y축 `mode_score`·
`harmonic_ratio`·밝기군[`centroid`/`rolloff`/`zcr`/`flatness`]·`flux`·`energy`·`bpm`, 실제 컬럼 10개)의
**밴드별 분포**를 `side-project/band-audio-analysis/`에서 플롯 + 분석(원본 오디오 재추출 불필요 —
`song_features_with_proxies.csv`·`audio_map.json`에 이미 다 있음). 목적은 ave_mujica 같은 "채널 판정이
밴드 특성과 어긋나는" 사례가 더 있는지 확인 후 `add_pulse_shape.py`의 판정 규칙을 손볼지 결정. 상세는
HANDOFF "열린 결정" § 세션 36 참조.
