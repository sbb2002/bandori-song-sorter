...
현 `src/content/cluster/audio_map.json`의 x·y는 **두 겹**으로 되어 있고, 이게 사용자 체감과 어긋난다:
...
구현 정제(확정):
1. **Peak f0 = 절대 최댓값이 아니라 전곡 유성음 f0의 상위 백분위(90~95p)** — 절대 max는 f0 추출 옥타브 오류에 취약. voicing 신뢰도 임계 + median filter로 옥타브 점프 제거. **클라이막스를 위치로 찾지 않고 이 백분위로 포착**(§2.1).
...
⚠️ **정정(2026-07-01)**: 현 캐시는 60초 구간이지만 **이미 48kHz mono**로 추출됨(`build_audio_map.py` `CACHE_SR=48000`, CLAP이 48k 요구). 16kHz는 캐시가 아니라 **librosa 특징추출 단계 내부 로드값**(`LIBROSA_SR=16000`)이었음. → **샘플레이트는 충분**, 재추출이 필요한 진짜 이유는 **구간(60초 고정, 45~105s)** 하나뿐.

### 2.1 클라이막스·구간 전략 — "위치를 찾지 않고 통계로 포착"
- **후렴/클라이막스를 탐지해 크롭하지 않는다(랜덤 크롭도 아님).** 대신 **전곡 보컬 f0를 뽑아 상위 백분위(95p)** 를 취함 → 최고음이 곡 어디에 있든(J-pop은 대개 조옮김된 **라스트 코러스**가 정점) 그 음이 자동 포착. **후렴 위치 탐지 불필요 → 탐지 오류 없음.**
- 현 45~105초 **고정창의 약점**: 대개 **첫 후렴**에 걸려 진짜 정점인 **마지막 후렴**을 놓침. → 전곡 통계가 이를 해결.
- **centroid/rolloff(밝기)** 는 정점이 아니라 곡 대표 음색 → **전곡 평균**.
- **재추출**: "60초 크롭" → **전곡(인트로·아웃트로 약간 제외 가능)** 48kHz. `ensure_audio()`의 `SECTION`/`--download-sections` 조정.
- **비용**: 전곡 Demucs+f0가 부담이면 **pYIN(CPU 친화)** 또는 인트로/아웃트로만 컷. CREPE는 정확하나 무거움.
- (선택) **pychorus / SSM(자기유사행렬)** 후렴 탐지로 교차검증 가능하나 오류 있어 **필수 아님**(백분위가 1차, 탐지는 보조).
...
- **결정적 장애물(데이터)**: 곡별 감성 = 곡별 가사 필요. **이 장치엔 가사 0개**(`assets/lyrics/<band>.md`는 gitignore, 템플릿만 존재). 템플릿=TOP10뿐이라 **캡40(~300곡)은 ~200곡분 가사 수작업 수집** 선결. 밴드 합산 `src/content/wordcloud/<band>.yaml`로는 곡별 분해 불가.
- 자원: `src/tools/wordcloud/senti_lexicon.yaml`(키워드 극성 +2~−2, 122개, 한글 기준). 파이프라인 JP가사→명사→KO번역→사전 매칭(손실 있음). 연주곡 결측.
...
2. **[매니페스트]** `build_audio_map.py`에 `--manifest`/캡 인자 추가 → `src/content/songs/*.yaml`에서 밴드당 40곡 샘플 생성(dedup=vid).
3. **[재추출]** 캐시는 이미 48kHz mono → **전곡으로 재빌드**(60초 크롭 폐기, `SECTION`/`--download-sections` 조정). 클라이막스는 전곡 f0 95p로 통계 포착(§2.1).
