...
> 축 재정의(30곡 파일럿 검증)는 **완료**(done 세션 21). 축·좌표 설계 근거는 [audio-map-axes.md](audio-map-axes.md) + `docs/report/cluster-correlation/README.md`. 여기는 **스케일업만** 다룸.
...
- **파이프라인**: `python tools/cluster/build_perceptual_map.py [--cache audio_cache|audio_full]` → `cluster/audio_map.json`(커밋).
  - 입력 매니페스트 = **`cluster/songs_top10.csv`**(하드코딩 `MANIFEST`, 컬럼 `band,idx,song,url`).
  - 음원 = `cluster/<cache>/<band>__<idx:03d>.wav`(48kHz, gitignore). 현재 `audio_cache`=60초 클립 TOP10×10 **97곡만** 존재.
...
### 2. 매니페스트 생성
`data/*.yaml`에서 **전체/캡 매니페스트 CSV**(`band,idx,song,url`, dedup=vid) 생성. `build_perceptual_map.py`에 **`--manifest` 인자 추가**(현재 `MANIFEST` 하드코딩) 또는 새 CSV로 교체. 캡 옵션이면 밴드당 상한 N 잘라내기.

### 3. 오디오 추출
매니페스트의 각 url을 **yt-dlp로 추출 → `cluster/audio_full/<band>__<idx:03d>.wav`**(48kHz, gitignore).
...
### 4. 빌드
`python tools/cluster/build_perceptual_map.py --cache audio_full [--manifest <full.csv>]` → `cluster/audio_map.json` → `python build.py`.
