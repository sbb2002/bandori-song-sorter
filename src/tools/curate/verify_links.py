...
검수 *보조* 도구입니다. 의심 항목만 골라 표로 뽑아줄 뿐, 데이터(src/content/songs/*.yaml)는
...
사용법
  python src/tools/curate/verify_links.py                 # Layer 0 (오프라인)
  python src/tools/curate/verify_links.py --oembed        # + 죽은 링크/제목 대조
  python src/tools/curate/verify_links.py --length        # + 풀버전 판별
  python src/tools/curate/verify_links.py --all           # 전부
  python src/tools/curate/verify_links.py --all --json tools/curate/verify_report.json
"""
...
CACHE_PATH = ROOT / "src" / "tools" / "curate" / "verify_cache.json"
...
def load_tracks():
    """Return a flat list of track dicts across every src/content/songs/*.yaml."""
    tracks = []
