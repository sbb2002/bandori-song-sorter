...
def main(argv=None) -> int:
    ...
    if rc != 0:
        print(f"\n✗ 오류 종료(rc={rc}) — 위 로그에서 실패 원인을 확인하세요. 데브 레포는 무변경.")
        return rc

    if "신곡 없음 — 종료(멱등)." in output:
        print("\nℹ️ 신곡 0건 — 처리할 게 없어 종료했습니다(commit/push 없음, 정상).")
        return 0

    m = re.search(r"반영 (\d+)곡 · 실패 (\d+)곡", output)
    if not m:
        print(f"\n종료(rc={rc}). 결과 메시지를 못 찾음 — 위 로그 직접 확인 권장.")
        return rc

    landed, failed = int(m.group(1)), int(m.group(2))
    if landed > 0:
        tail = f" (실패 {failed}곡 — 다음 실행에서 재시도됨)" if failed else ""
        print(f"\n✅ {landed}곡 반영 완료{tail} — main push 시 deploy.yml 이 라이브 재배포(자동).")
    else:
        print(f"\n⚠️ 신곡 {failed}곡을 시도했지만 전부 실패 — 반영 없음(push 안 됨). 위 로그 확인.")
    return rc
