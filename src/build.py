...
# 경로는 CWD가 아니라 이 파일 위치(src/)를 기준으로 잡는다 → 어느 폴더에서 실행해도 동작.
SRC = Path(__file__).resolve().parent          # .../src
ROOT = SRC.parent                              # 레포 루트 (배포물 index.html 위치)
CONTENT = SRC / "content"                      # 빌드 입력: songs/·wordcloud/·cluster/


def load_senti(path=SRC / "tools" / "wordcloud" / "senti_lexicon.yaml"):
...
def load_wordclouds(wc_dir=CONTENT / "wordcloud"):
    """content/wordcloud/<band>.yaml → {band: {song_count, keywords:[{jp,ko,weight,senti}]}}.
...
def load_cluster(path=CONTENT / "cluster" / "audio_map.json"):
    """밴드 음원 지도 좌표(build_perceptual_map.py 산출). 없으면 빈 dict(맵 탭 비활성).
...
def build():
    ...
    - 산출 index.html 은 레포 루트에 쓴다(GitHub Pages 서빙 위치).
    """
    yaml_dir = CONTENT / "songs"
